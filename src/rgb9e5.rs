...
#[inline]
pub fn vec3_to_rgb9e5(rgb: &[f32]) -> u32 {
...
#[inline]
pub fn vec3_to_rgb9e5_roundup(rgb: &[f32]) -> u32 {
    let rc = rgb[0].clamp(0.0, MAX_RGB9E5);
    let gc = rgb[1].clamp(0.0, MAX_RGB9E5);
    let bc = rgb[2].clamp(0.0, MAX_RGB9E5);
    ...
    let rm = ((rc / denom).ceil() as i32).min(MAX_RGB9E5_MANTISSA);
    let gm = ((gc / denom).ceil() as i32).min(MAX_RGB9E5_MANTISSA);
    let bm = ((bc / denom).ceil() as i32).min(MAX_RGB9E5_MANTISSA);
